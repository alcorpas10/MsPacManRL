...
def q_learning_replay(model, episodes=100, port=38514, gamma=0.7, epsilon=0.2, replay_size=32, memory_size=10000, title='DQN Replay'):
    ...
    game = Game(num_episodes=episodes, port=port) # create the game class  with the number of episodes and the port
    ...
    for episode in range(episodes): # for each episode
        ...
        while True: # while the game is not over
            ...
            if random.random() < epsilon:   #if random number is less than epsilon, explore
...
def main():
    ...
    model = DQN_replay(hidden_dim=int(args[3]), lr=0.001, mom=0.9)  # create the model with the given hidden_dim
