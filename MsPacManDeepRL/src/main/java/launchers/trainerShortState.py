...
class DQN():
    ...
    def __init__(self, state_dim=4, action_dim=4, hidden_dim=8, lr=0.0005, mom=0.9):
...
def q_learning_replay(model, episodes=100, port=38514, gamma=0.7, epsilon=0.2, replay_size=32, memory_size=10000, title='DQN Replay'):
    ...
    for episode in range(episodes): # for each episode
        ...
        while True:
            ...
            # Take next_state ,action and  reward from the game engine
...
def main():
    ...
    args = sys.argv[1:] 
    # initialize the model with the third argument as the hidden layer size
